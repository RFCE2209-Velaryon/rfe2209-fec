...
const CompareModal = (props) => {
  ...
  return (
    <div onClick={()=>props.onClick()} style={{backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', position:'fixed', zIndex: 10, left: 0, top: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center'}} className='modalWrapper'>
      <div style={{backgroundColor: 'white', position:'relative', width: 405+'px', border: 1 + 'px solid black'}} className='modalContent'>
        <table>
          ...
          {props.allFeatures.map((feat, i)=> {
            return (<tr key = {i}><td style={{textAlign: 'center', width:135+'px'}}>{feat[0]}</td><td style={{textAlign: 'center', width:135+'px'}}>{feat[1]}</td><td style={{textAlign: 'center', width:135+'px'}}>{feat[2]}</td></tr>);
          })}
        </table>
      </div>
    </div>
